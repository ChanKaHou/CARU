...
trainData, validData, testData = torchtext.datasets.Multi30k.splits(root='../.data/', exts=('.de', '.en'), fields=(de, en))
print('Dataset Size:', len(trainData.examples), len(validData.examples), len(testData.examples))

de.build_vocab(trainData.src, min_freq=4)
en.build_vocab(trainData.trg, min_freq=4)
print('Vocabulary Size:', len(de.vocab), len(en.vocab))
...
if os.path.exists('Multi30k-de2en-CARUCell.pkl'):
    print("Loaded Multi30k-de2en-CARUCell.pkl")
    pkl = torch.load('Multi30k-de2en-CARUCell.pkl')
...
while (scheduler.last_epoch < 100):
    ...
    torch.save(
        ...
        'Multi30k-de2en-CARUCell-%02d-(%.04f,%.04f,%.04f).pkl' %(scheduler.last_epoch, trainLoss, validBLEU, testBLEU)
        )
